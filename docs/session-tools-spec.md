...
Генерация музыки живёт рядом, отдельной спекой: [music-tools-spec.md](music-tools-spec.md). Инструменты `generate-music`, `music-status`, `music-credits` (`packages/mcp-server/src/tools/music/`) делают трек в Suno через прокси-провайдера, скачивают mp3 и кладут его в Data-каталог **тем же** запросом `foundry-mcp-bridge.files.upload`, что и `upload-file`, а в плейлист - через `manage-playlists` (`create` / `add-tracks`). Модуль Foundry для этого не меняется. В манифесте `session-apply.mjs` трек плейлиста может нести ключ `generate` вместо готового файла.
